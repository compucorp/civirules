...
class CRM_CivirulesCronTrigger_WeeklyGroupMembership extends CRM_Civirules_Trigger_Cron {
  ...
  protected function getNextEntityTriggerData() {
    if ($this->triggerParams['week_day'] != date('w')) {
      return FALSE;
    }
    $currentTime = explode(':', date('H:i'));
    $currentTime = ($currentTime[0] * 60) + $currentTime[1];
    if (!empty($this->triggerParams['not_before_time'])) {
      $notBeforeTime = explode(':', $this->triggerParams['not_before_time']);
      $notBeforeTime = ($notBeforeTime[0] * 60) + $notBeforeTime[1];
      if ($currentTime < $notBeforeTime) {
        return FALSE;
      }
    }
    if (!empty($this->triggerParams['not_after_time'])) {
      $notAfterTime = explode(':', $this->triggerParams['not_after_time']);
      $notAfterTime = ($notAfterTime[0] * 60) + $notAfterTime[1];
  ...
  public function getExtraDataInputUrl($ruleId) {
    return CRM_Utils_System::url('civicrm/civirule/form/trigger/weeklygroupmembership/', 'rule_id=' . $ruleId);
  }
  ...
  public function getTriggerDescription(): string {
    ...
    $weekDays = CRM_Utils_Date::getFullWeekdayNames();
    $weekDay = $weekDays[$this->triggerParams['week_day']];

    $time = '';
    if (!empty($this->triggerParams['not_before_time']) && !empty($this->triggerParams['not_after_time'])) {
      $time = E::ts('Between %1 and %2', [1=>$this->triggerParams['not_before_time'], 2=>$this->triggerParams['not_after_time']]);
    } elseif (!empty($this->triggerParams['not_before_time'])) {
      $time = E::ts('After %1', [1=>$this->triggerParams['not_before_time']]);
    } elseif (!empty($this->triggerParams['not_after_time'])) {
      $time = E::ts('Before %1', [1=>$this->triggerParams['not_after_time']]);
    }
    return trim(E::ts('Weekly trigger for all members of group %1 on each %2. %3', [
      1 => $groupName,
      2 => $weekDay,
      3 => $time,
    ]));
